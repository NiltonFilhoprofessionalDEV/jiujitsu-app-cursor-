...
-- Sensitive medical / emergency data — staff or self only (not on academy_members).
...
-- Fan-out notifications for other profiles (bypasses notif_own RLS safely).
...
-- Atomic approve: mark request + insert attendance record in one transaction.
...
-- No direct INSERT on academies: use create_academy_with_owner RPC only.
...
-- Private details: self or staff (assistant_instructor can read; write is instructor+)
